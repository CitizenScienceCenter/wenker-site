...
export default {
  ...
  methods: {
    ...
    startProject() {
      if (
        this.userDetails.age !== undefined &&
        this.userDetails.canton !== undefined
      ) {
        console.log(this.user)
        this.$store
          .dispatch('user/updateUser', [
            this.user.id,
            { info: this.userDetails }
          ])
          .then(res => {
            console.log(this.project)
            if (
              this.project.info &&
              this.project.info.task_selection === 'linear'
            ) {
              this.$store
                .dispatch('project/getTasks', [
                  this.project.id,
                  1,
                  this.progress
                ])
              ...
              this.$store
                .dispatch('task/randomProjectTask', [
                  this.project.id,
                  this.userDetails.canton
                ])
                .then(task => {
                  console.log(task)
                  this.$router.push({
