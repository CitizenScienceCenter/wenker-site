              ...
              <div class="form" v-if="responses.length">
                <select v-model="activeAnswerIndex">
                 <option v-for="(r, index) in task.content.answers" :value="index" :key="index">{{index+1}}</option>
                  ...
                  <upload v-if="activeAnswer.type.indexOf('file') !== -1" :embedded="true" :multiple="activeAnswer.type === 'multiple_files'"></upload>
                  <div v-if="activeAnswer.type === 'text'" class="form-field">
                    <label for="qutxt">{{activeAnswer.placeholder || "In Ihrem Dialekt"}}</label>
                    <input type="text" v-on:focus="focus(i)" v-on:keyup="changeNext" v-model="responses[activeAnswerIndex].text" name="qutxt" id="qutxt" />
                  </div>
                  <submission-multiple-choices :index="i" :content="responses[activeAnswerIndex]" :choices="answer.choices" :type="activeAnswer.type" v-if="activeAnswer.type === 'multiple_choice'"></submission-multiple-choices>
                </div>
                <div class="buttons">
...
export default {
  ...
  data() {
    return {
      ...
      focussed: undefined,
      task_type: 'Bogen',
      task_help: `<h4>Transkriptionskonventionen</h4>
            <ul>
  ...
  components: { Upload, SubmissionMultipleChoices, ProjectInfo, InfoPopup, CommentsList },
  computed: {
    ...mapState({
  ...
  watch: {
    ...
    taskMedia(to, from) {
      if (to.length > 0) {
        const m = to[0]
        const path = m.path.replace('./static', process.env.HOST + '/files')
        this.img = path
        console.log(path)
  ...
  mounted() {
    ...
    this.activeAnswer = this.task.content.answers[this.activeAnswerIndex]
    this.createSubmission()
  },
  methods: {
    updateActiveIndex(val) {
      const newIndex = this.activeAnswerIndex + val
      if (newIndex >= 0 && newIndex < this.task.content.answers.length) {
        this.activeAnswerIndex = this.activeAnswerIndex + val
      }
    },
    createSubmission() {
      const submission = {
